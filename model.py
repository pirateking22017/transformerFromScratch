...
class LayerNormalzation(nn.Module):
    ...
    def forward(self, x):
        ...
        std = x.std(dim=-1, keepdim = True) #std deviation
...
class PostionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        position = torch.arange(0, seq_len, dtype = torch.float).unsqueeze(1) #what we've just done is create a tensor of seq_len = 1
    ...
    def forward(self, x):
        x = x + (self.pe[:, :x.shape[1], :]).requires_grad_(False)# we hv to tell the model, that dont learn pe since theyre fixed -- req_grad does tht
...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout:float) -> None:
        ...
        assert d_model % h == 0, "d_model isnt divisible by the number of heads"
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        if mask is not None:
            attention_scores.masked_fill(mask == 0, -1e9) #wherer mask == 0, replace 0 with -1e9 which is a vv small value 
